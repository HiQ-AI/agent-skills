...
| 需求 | MCP 工具 | CLI 命令 |
|---|---|---|
| 材料 / 工艺 → 数据集 key | *(无,走 REST)* | `hiq-cortex search "<原话>" [--sources X]` |
| key → GWP + 基准 | `lookup_datasets` | `hiq-cortex lookup-datasets --dataset-keys <key[,key…]>` |
| **非 GWP 的 LCIA 指标**(AP/EP/ODP/WDP/ADP) | `aggregate_indicators` | `hiq-cortex aggregate-indicators --dataset-keys <keys> --indicator AP --source X` |
| 队列分布 / 百分位定位 | `aggregate_datasets` | `hiq-cortex aggregate-datasets --where '{"sources":["X"]}' [--target-value N]` |
| **工序级热点** | `process_hotspot` | `hiq-cortex process-hotspot --dataset-key <key> [--indicator GWP100]` |
