...
**本技能只查 ecoinvent。** 每次 `search` 都要带 `--sources Ecoinvent`,聚合/指标类带
...
| 需求 | MCP 工具 | CLI 命令 |
|---|---|---|
| 材料名 → 数据集 key | *(无,走 REST)* | `hiq-cortex search "<原话>" --sources Ecoinvent` |
| key → GWP + 基准 | `lookup_datasets` | `hiq-cortex lookup-datasets --dataset-keys <key[,key…]>` |
| 队列 GWP 分布 / 百分位定位 | `aggregate_datasets` | `hiq-cortex aggregate-datasets --where '{"sources":["ecoinvent"]}' [--target-value N]` |
| 非 GWP 的 LCIA 指标 | `aggregate_indicators` | `hiq-cortex aggregate-indicators --dataset-keys <keys> --indicator AP --source ecoinvent` |
| 单数据集工序级热点 | `process_hotspot` | `hiq-cortex process-hotspot --dataset-key <key>` |
