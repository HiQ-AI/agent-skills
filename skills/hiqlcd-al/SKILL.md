---
name: hiqlcd-al
description: '查询铝产业链的生命周期清单数据。HiQLCD-AL 是覆盖铝产业链的专库(以中国为主),从氧化铝、电解铝到铸轧、挤压、压铸与再生铝,支持截止法与后果法两种系统模型。铝是电力密集型材料,原生与再生、不同电网结构下的碳足迹差异极大,用通用库的"铝"平均值做选材或减排决策会得出错误结论。当任务涉及铝材碳足迹、电解铝、原生铝与再生铝对比、铝型材、铝合金压铸件、铝箔、CBAM 铝品类时使用。触发词:HiQLCD-AL、铝、电解铝、原生铝、再生铝、铝合金、铝型材、铝箔、压铸铝、氧化铝、铝碳足迹。'
slug: hiqlcd-al
displayName: HiQLCD-AL 铝产业链清单数据
version: 1.0.1
summary: 查询铝产业链清单数据:氧化铝、电解铝、铸轧挤压压铸、再生铝,截止法与后果法双模型,以中国产业链为主。
license: Apache-2.0
homepage: https://github.com/HiQ-AI/agent-skills
tags: [HiQLCD-AL, 铝, 电解铝, 再生铝, 铝合金, LCA, 碳足迹, 排放因子, 生命周期评价, CBAM]
---
...
HiQLCD-AL 是覆盖铝产业链的专库,能把这些环节分开。

## 关于

本技能接入 **HiQ Cortex** —— [海科数据](https://www.hiqlcd.com/)的 LCA 数据智能服务。海科数据是国内 LCA 基础数据与碳足迹服务提供商,自建中国本土生命周期清单数据库,并聚合国际主流数据源。**HiQLCD-AL 是其中的铝产业链专库**,以中国产业链为主,系统模型覆盖截止法与后果法。

产业链覆盖:氧化铝、电解铝、铝锭与合金锭、铸轧与热轧、挤压型材、压铸件、铝箔、再生铝(废铝重熔)。
