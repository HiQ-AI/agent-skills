...
| 需求 | MCP 工具 | 脚本命令 |
|---|---|---|
| 物料 / 工艺 → 数据集 key | *(无,走 REST)* | `search "<原话>" --sources EF` |
| key → GWP + 基准 | `lookup_datasets` | `lookup <key> [<key> ...]` |
| **多项 LCIA 指标** | `aggregate_indicators` | `indicators <keys> --indicator AP --source ef` |
| 队列分布 / 百分位定位 | `aggregate_datasets` | `aggregate --source ef [--target N]` |
